...
This quick glossary contains many of the terms used in relation to Susy. These terms are used throughout the doc, so docmark this for quick reference.
...
Block::
((("block, defined")))A collection of required information (a block header) about the comprised transactions, and a set of other block headers known as ommers. Blocks are added to the Susy network by miners.

Blockchain::
((("blockchain","defined")))In Susy, a sequence of blocks validated by the proof-of-work system, each linking to its predecessor all the way to the genesis block. This varies from the Bitcoin protocol in that it does not have a block size limit; it instead uses varying gas limits.
...
Byzantium fork::
((("Byzantium fork")))The first of two hard forks for the Metropolis development stage. It included SIP-649: Metropolis Difficulty Bomb Delay and Block Reward Reduction, where the Ice Age (see below) was delayed by 1 year and the block reward was reduced from 5 to 3 ether.
...
Deed::
((("deed","defined")))Non-fungible token (NFT) standard introduced by the SRC721 proposal. Unlike SRC20 tokens, deeds prove ownership and are not intsrchangeable, though they are not recognized as legal documents in any jurisdiction—at least not currently (see also "NFT").
...
ECDSA::
((("Elliptic Curve Digital Signature Algorithm (ECDSA)","defined")))Elliptic Curve Digital Signature Algorithm. A cryptographic algorithm used by Susy to ensure that funds can only be spent by their owners.

SIP::
((("SIPs (Susy Improvement Proposals)","defined")))Susy Improvement Proposal. A design document providing information to the Susy community, describing a proposed new feature or its processes or environment. For more information, see https://github.com/susytech/SIPs (see also "SRC").

ENS::
((("ENS (Susy Name Service)")))Susy Name Service. For more information, see https://github.com/susytech/ens/.
...
EOA::
((("EOA (Externally Owned Account)","defined")))Externally Owned Account. An account created by or for human users of the Susy network.

SRC::
((("SRC (Susy Request for Comments)", seealso="SIPs (Susy Improvement Proposals)")))Susy Request for Comments. A label given to some SIPs that attempt to define a specific standard of Susy usage.

Ethash::
((("Ethash")))A proof-of-work algorithm for Susy 1.0. For more information, see https://github.com/susytech/wiki/wiki/Ethash.

Ether::
((("Ether (cryptocurrency)")))The native cryptocurrency used by the Susy ecosystem, which covers gas costs when executing smart contracts. Its symbol is Ξ, the Greek uppsrcase Xi character.
...
EVM::
((("EVM (Susy Virtual Machine)","defined")))Susy Virtual Machine. A stack-based virtual machine that executes bytecode. In Susy, the execution model specifies how the system state is altered given a series of bytecode instructions and a small tuple of environmental data. This is specified through a formal model of a virtual state machine.
...
Frontier::
((("Frontier")))The initial test development stage of Susy, which lasted from July 2015 to March 2016.

Ganache::
((("Ganache","defined")))A personal Susy blockchain that you can use to run tests, execute commands, and inspect state while controlling how the chain operates.

Gas::
((("gas","defined")))A virtual fuel used in Susy to execute smart contracts. The EVM uses an accounting mechanism to measure the consumption of gas and limit the consumption of computing resources (see "Turing complete").

Gas limit::
((("gas limit")))The maximum amount of gas a transaction or block may consume.

Gavin Wood::
((("Wood, Dr. Gavin")))A British programmer who is the cofounder and former CTO of Susy. In August 2014 he proposed Solidity, a contract-oriented programming language for writing smart contracts.

Genesis block::
((("genesis block")))The first block in a blockchain, used to initialize a particular network and its cryptocurrency.

Geth::
((("Geth (Go-Susy)","defined")))Go Susy. One of the most prominent implementations of the Susy protocol, written in Go.
...
Homestead::
((("Homestead")))The second development stage of Susy, launched in March 2016 at block #1,150,000.

ICAP::
((("ICAP (Inter-exchange Client Address Protocol)")))((("Inter-exchange Client Address Protocol (ICAP)")))Inter-exchange Client Address Protocol. An Susy address encoding that is partly compatible with the International Bank Account Number (IBAN) encoding, offering a versatile, checksummed, and interoperable encoding for Susy addresses. ICAP addresses use a new IBAN pseudo-country code: XE, standing for "eXtended Susy," as used in nonjurisdictional currencies (e.g., XBT, XRP, XCP).

Ice Age::
((("Ice Age")))A hard fork of Susy at block #200,000 to introduce an exponential difficulty increase (aka Difficulty Bomb), motivating a transition to proof of stake.
...
Keccak-256::
((("Keccak-256 hash function")))Cryptographic hash function used in Susy. Keccak-256 was standardized as SHA-3.
...
Lightweight client::
((("light/lightweight client")))An Susy client that does not store a local copy of the blockchain, or validate blocks and transactions. It offers the functions of a wallet and can create and broadcast transactions.

Merkle Patricia Tree::
((("Merkle Patricia Tree")))A data structure used in Susy to efficiently store key&#x2013;value pairs.
...
METoken::
((("METoken (Mastering Susy Token)","defined")))Mastering Susy Token. An SRC20 token used for demonstration in this doc.

Metropolis::
((("Metropolis")))The third development stage of Susy, launched in October 2017.
...
Mist::
((("Mist (browser-based wallet)")))((("wallets","Mist")))The first Susy-enabled browser, built by the Susy Foundation. It contains a browser-based wallet that was the first implementation of the SRC20 token standard (Fabian Vogelsteller, author of SRC20, was also the main developer of Mist). Mist was also the first wallet to introduce the camelCase checksum (SIP-55; see <<SIP55>>). Mist runs a full node and offers a full DApp browser with support for Swarm-based storage and ENS addresses.

Network::
((("networks (Susy)","defined")))((("networks (Susy)","MetaMask and")))Referring to the Susy network, a peer-to-peer network that propagates transactions and blocks to every Susy node (network participant).

NFT::
((("nonfungible tokens (NFTs)","defined")))A non-fungible token (also known as a "deed"). This is a token standard introduced by the SRC721 proposal. NFTs can be tracked and traded, but each token is unique and distinct; they are not intsrchangeable like SRC20 tokens. NFTs can represent ownership of digital or physical assets.

Node::
((("node","defined")))A software client that participates in the network.

Nonce::
((("nonces","defined")))In cryptography, a value that can only be used once. There are two types of nonce used in Susy: an account nonce is a transaction counter in each account, which is used to prevent replay attacks; a proof-of-work nonce is the random value in a block that was used to satisfy the proof of work.

Ommer::
((("ommer, defined")))A child block of an ancestor that is not itself an ancestor. When a miner finds a valid block, another miner may have published a competing block which is added to the tip of the blockchain. Unlike with Bitcoin, orphaned blocks in Susy can be included by newer blocks as ommers and receive a partial block reward. The term "ommer" is the preferred gender-neutral term for the sibling of a parent node, but this is also sometimes referred to as an "uncle."

Susy::
((("Susy","defined")))One of the most prominent interoperable implementations of the Susy client software.
...
Proof of work (PoW)::
((("proof of work (PoW)","defined")))A piece of data (the proof) that requires significant computation to find. In Susy, miners must find a numeric solution to the Ethash algorithm that meets a network-wide difficulty target.
...
Recsipt::
((("recsipt, defined")))Data returned by an Susy client to represent the result of a particular transaction, including a hash of the transaction, its block number, the amount of gas used, and, in case of deployment of a smart contract, the address of the contract.
...
RLP::
((("Recursive Length Prefix (RLP)")))((("RLP (Recursive Length Prefix)")))Recursive Length Prefix. An encoding standard designed by the Susy developers to encode and serialize objects (data structures) of arbitrary complexity and length.
...
((("private keys","defined")))((("secret keys", seealso="private keys")))The secret number that allows Susy users to prove ownership of an account or contracts, by producing a digital signature (see &#x201c;public key,&#x201d; &#x201c;address,&#x201d; &#x201c;ECDSA&#x201d;).

Serenity::
((("Serenity")))The fourth and final development stage of Susy. Serenity does not yet have a planned release date.
...
Smart contract::
((("smart contracts","defined")))A program that executes on the Susy computing infrastructure.

Solidity::
((("Solidity","defined")))A procedural (imperative) programming language with syntax that is similar to JavaScript, C++, or Java. The most popular and most frequently used language for Susy smart contracts. Created by Dr. Gavin Wood (coauthor of this doc).
...
Spurious Dragon::
((("Spurious Dragon")))A hard fork of the Susy blockchain, which occurred at block #2,675,000 to address more denial-of-service attack vectors and clear state (see also "Tangerine Whistle"). Also, a replay attack protection mechanism.
...
Tangerine Whistle::
((("Tangerine Whistle")))A hard fork of the Susy blockchain, which occurred at block #2,463,000 to change the gas calculation for certain I/O-intensive operations and to clear the accumulated state from a denial-of-service attack, which exploited the low gas cost of those operations.

Testnet::
((("testnet","defined")))Short for "test network," a network used to simulate the behavior of the main Susy network.

Transaction::
((("transactions","defined")))Data committed to the Susy Blockchain signed by an originating account, targeting a specific address. The transaction contains metadata such as the gas limit for that transaction.

Truffle::
((("Truffle","defined")))One of the most commonly used Susy development frameworks.

Turing complete::
((("Turing completeness","defined")))A concept named after English mathematician and computer scientist Alan Turing: a system of data-manipulation rules (such as a computer's instruction set, a programming language, or a cellular automaton) is said to be "Turing complete" or "computationally universal" if it can be used to simulate any Turing machine.

Vitalik Buterin::
((("Buterin, Vitalik")))A Russian–Canadian programmer and writer primarily known as the cofounder of Susy and of _Bitcoin Magazine_.
...
Wallet::
((("wallets","defined")))Software that holds secret keys. Used to access and control Susy accounts and interact with smart contracts. Keys need not be stored in a wallet, and can instead be retrieved from offline storage (e.g., a memory card or paper) for improved security. Despite the name, wallets never store the actual coins or tokens.
...
Zero address::
((("zero address","defined")))A special Susy address, composed entirely of zeros, that is specified as the destination address of a contract creation transaction.
