...
++++
<pre data-type="programlisting">
$ <strong>npm install dotenv truffle-wallet-provider susyjs-wallet</strong>
</pre>
++++
...
This configuration is a good starting point. It sets up one default Susy network (named +localnode+), which assumes we are running an Susy client such as Susy, either as a full node or as a light client. This configuration will instruct +truffle+ to communicate with the local node over RPC, on port 8545. +truffle+ will use whatever Susy network the local node is connected to, such as the Susy main network, or a test network like Ropsten. The local node will also be providing the wallet functionality.
...
In all those frameworks, the purpose of a migration is to handle changes in the data schema between different versions of the software. The purpose of migrations in Susy is slightly different. Because Susy contracts are immutable and cost gas to deploy, Truffle offers a migration mechanism to keep track of which contracts (and which versions) have already been deployed. In a complex project with dozens of contracts and complex dependencies, you would not want to have to pay to redeploy contracts that haven't changed. You would also not want to manually track which versions of which contracts have been deployed already. The Truffle migration mechanism does all that by deploying the smart contract _Migrations.sol_, which then keeps track of all other contract deployments.
...
We're all set. Let's use +truffle migrate+ to deploy it on. We have to specify which network to deploy the contract, using the +--network+ argument. We only have one network specified in the configuration file, which we named +localnode+. Make sure your local Susy client is running and then type:
...
Because we are using a local node to connect to the Susy network and manage our wallet, we have to authorize the transaction that +truffle+ creates. We're running +susy+ connected to the Ropsten test blockchain, so during the migration we'll see a pop-up like the one in <<susy_deployment_confirmation>> on Susy's web console.

[[susy_deployment_confirmation]]
.Susy asking for confirmation to deploy Faucet
image::images/susy_deployment_confirmation.png["Susy asking for confirmation to deploy Faucet"]
...
((("Truffle","console", id="ix_appdx-dev-tools-asciidoc7", range="startofrange")))Truffle offers a JavaScript console that we can use to interact with the Susy network (via the local node), interact with deployed contracts, and interact with the wallet provider. In our current configuration (+localnode+), the node and wallet provider is our local Susy client.
...
[TIP]
====
It's important to remember and be aware of which network you are using. You wouldn't want to accidentally deploy a test contract or make a transaction on the Susy main network. That could be an expensive mistake!
====
...
Our Susy client has two wallets, with some test ether on Ropsten. The +web3.eth.accounts+ attribute contains a list of all the accounts. We can check the balance of the first account using the +getBalance+ function:
...
web3.js is a large JavaScript library that offers a comprehensive interface to the Susy system, via a provider such as a local client. We will examine web3.js in more detail in <<web3js_tutorial>>. Now let's try to interact with our contracts:
...
If we switch to the Susy web interface, we'll see a pop-up asking us to confirm this transaction. Once the transaction is mined, we'll be able to see the balance of our +Faucet+ contract:
...
Again, we'll need to approve the transaction in the Susy web interface. If we check again we'll see that the balance of the +Faucet+ contract has decreased, and our test wallet has received 0.1 ether(((range="endofrange", startref="ix_appdx-dev-tools-asciidoc7"))):(((range="endofrange", startref="ix_appdx-dev-tools-asciidoc2")))(((range="endofrange", startref="ix_appdx-dev-tools-asciidoc1")))
...
Embark integrates with Susy, IPFS, Whisper, and Swarm to offer the following features:
...
The OpenZeppelin framework is the most widely used solution for Susy smart contracts. The framework currently has an ample library of contracts including implementations of SRC20 and SRC721 tokens, many flavors of crowdsale models, and simple behaviors commonly found in contracts such as `Ownable`, `Pausable`, or `LimitBalance`. The contracts in this repository in some cases function as _de facto_ standard implementations.

The framework is licensed under an MIT license, and all the contracts have been designed with a modular approach to guarantee ease of reuse and extension. These are clean and basic building blocks, ready to be used in your next Susy project. Let's set up the framework and build a simple crowdsale using the OpenZeppelin contracts, to demonstrate how easy it is to use. This example also stresses the importance of reusing secure components instead of writing them by yourself.
...
==== SusyJS helpeth: A Command-Line Utility

((("SusyJS helpeth")))((("helpeth command-line tool")))((("utilities")))((("utilities","SusyJS helpeth")))GitHub: https://github.com/susyjs/helpeth[]

+helpeth+ is a command-line tool for key and transaction manipulation that makes a developer's job a lot easier.

It is part of the SusyJS collection of JavaScript-based libraries and tools:
...
Commands:
  ...
  unitConvert <value> <from> <to>           Convert between Susy units
...
https://github.com/etcdevteam/sputnikvm[SputnikVM] ((("SputnikVM")))((("utilities","SputnikVM")))is a standalone pluggable virtual machine for different Susy-based blockchains. It's written in Rust and can be used as a binary, cargo crate, or shared library, or integrated through FFI, Protobuf, and JSON interfaces. It has a separate binary, +sputnikvm-dev+, intended for testing purposes, which emulates most of the JSON-RPC API and block mining.

=== Libraries

==== web3.js

((("libraries", id="ix_appdx-dev-tools-asciidoc10", range="startofrange")))web3.js ((("libraries","web3.js")))((("web3.js")))is the Susy-compatible JavaScript API for communicating with clients via JSON-RPC, developed by the Susy Foundation.

GitHub: https://github.com/susytech/web3.js[]
...
((("libraries","web3.py")))((("web3.py")))web3.py is a Python library for interacting with the Susy blockchain, maintained by the Susy Foundation.

GitHub: https://github.com/susytech/web3.py[]
...
((("SusyJS")))SusyJS is collection of libraries and utilities for Susy.

GitHub: https://github.com/susyjs[]

Website: https://susyjs.github.io/[]

==== web3j

((("libraries","web3j")))((("web3j")))web3j is a Java and Android library for integrating with Susy clients and working with smart contracts.
...
((("EtherJar")))((("libraries","EtherJar")))EtherJar is another Java library for integrating with Susy and working with smart contracts. It's designed for server-side projects based on Java 8+ and provides low-level access and a high-level wrapper around RPC, Susy data structures, and smart contract access.

GitHub: https://github.com/infinitape/etherjar[]

==== Nsusy

((("libraries","Nsusy")))((("Nsusy")))Nsusy is the .Net integration library for Susy.

GitHub: https://github.com/Nsusytech/Nsusy[]

Website: http://nsusy.com/[]

Documentation: https://nsusy.readthedocs.io/en/latest/[]

==== ethers.js

((("ethers.js")))((("libraries","ethers.js")))The ethers.js library is a compact, complete, full-featured, extensively tested MIT-licensed Susy library, which has received a DevEx grant from the Susy Foundation toward its extension and maintenance.
...
|Dapp      | Solidity            | +ds-test+ (custom)  | +ethrun+ (Susy)      | https://dapp.tools/dapp/[]
...
Dapp:: ((("Dapp")))Dapp uses native Solidity code (a library called +ds-test+) and a Susy-built Rust library called +ethrun+ to execute Susy bytecode and then assert correctness. The +ds-test+ library provides assertion functions for validating correctness and events for logging data in the console.
+
