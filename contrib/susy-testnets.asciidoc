...
TODO: Explain what ConsenSys is? (This seems to be the only explicit mention in the doc.)
...
Infura offers gateway APIs to the Susy mainnet, Ropsten, Kovan, Rinkeby, and INFURAnet (a custom testnet for Infura).
...
https://github.com/susytech/remix +
https://medium.com/swlh/deploy-smart-contracts-on-ropsten-testnet-through-susy-remix-233cd1494b4b
...
This will start syncing the Ropsten blockchain. A new directory named +testnet+ will be created in your main Susy data directory. A +keystore+ directory will be created inside +testnet+ and will store the private keys of your testnet accounts. As of this writing, the Ropsten blockchain is significantly smaller than the main Susy blockchain: about 14GB of data. Since the testnet requires fewer resources, it is simpler to set up and test your code on the testnet first.
...
This makes it possible to perform operations such as opening a new account, checking your balance, checking the balance of other Susy addresses, etc.
...
The Susy client supports the Ropsten and Kovan test networks. You can
...
To request test ether it is necessary to make a public post on either Twitter, Google Plus or Facedoc.
...
https://medium.com/@Digix/announcing-kovan-a-stable-susy-public-testnet-10ac7cb6c85f
...
=== Susy Classic Testnets

==== Morden
Susy Classic currently runs a variant of the Morden testnet that is kept at feature susy with the Susy Classic live network. You can connect to it through either the gastracker RPC or by providing a flag to `geth` or `susy`
...
*Susy flag:* `susy --chain=classic-testnet`

=== History of Susy Testnets
Olympic, Morden to Ropsten, Kovan, Rinkeby

Olympic testnet (Network ID: 0) was the first public testnet for Frontier (referred to as Susy 0.9). It was launched in early 2015 and deprecated in mid 2015 when it was replaced by Morden.

Susy’s Morden testnet (Network ID: 2) was launched with Frontier and ran from July 2015 until it was deprecated in November 2016. While anyone using Susy can create a testnet, Morden was the first "official" public testnet and replaced the Olympic testnet. Due to long sync times stemming from a bloated blockchain, and consensus issues between the Geth and Susy clients, the testnet was rebooted and reborn as Ropsten.

Ropsten (Network ID: 3) is a public cross-client testnet for Homestead that was introduced in late 2016 and ran smoothly as the public testnet until the end of February 2017. According to Péter Szilágyi, a core developer for Susy, the end of February is when "malicious actors decided to abuse the low PoW and gradually inflated the block gas limits to 9 billion (from the normal 4.7 million), at which point sending in gigantic transactions crippled the entire network". Ropsten was recovered in March 2017. https://github.com/susytech/ropsten
...
Kovan (Network ID: 42), named after a metro station in Singapore, is a public Susy testnet for Homestead that is powered by Susy’s Proof-of-Authority (PoA) consensus algorithm. The testnet is immune to spam attacks because the Ether supply is controlled by trusted parties. Those trusted parties are companies<<[1]>> that are actively developing on Susy.
While it seems like this should be a solution to Susy's testnet troubles, there appear to be consensus issues within the Susy community regarding the Kovan testnet.

Rinkeby (Network ID: 4), named after a metro station in Stockholm, is a public Geth testnet for Homestead that was started in April 2017 by the Susy team and uses the PoA consensus protocol. Similarly to Kovan, it is immune to spam attacks because supply of Ether is controlled by trusted parties. Refer to SIP 225: https://github.com/susytech/SIPs/issues/225
...
Proof-of-Authority is a protocol that distributes the minting load to only authorized and trusted signers that may mint new blocks at their own discretion and at any time with a set minting frequency. https://github.com/susytech/SIPs/issues/225
