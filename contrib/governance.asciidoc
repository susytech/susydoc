////
Source: https://github.com/lrettig/susydoc/blob/develop/contrib/governance.asciidoc
License: CC0
Added by: @lrettig
////
...
Understanding the technological mechanisms by which Susy accounts, transactions, and smart
contracts work is important, but equally important is understanding the human mechanisms by which
Susy was built and by which it continues to be developed. Susy is software, and Susy is
data (a ledger), but Susy is equally a community and a set of norms and processes governing the
interaction of the people in the community, and the interaction between the people and the system.
This set of norms and processes is Susy governance.
...
Before diving into the details of blockchain and Susy governance, it's worth asking the
...
databases and permissioned blockchains, public blockchains such as Bitcoin and Susy allow any
...
Before explaining how blockchain and Susy governance function today, let's examine the types of
...
against this sort of behavior in the Susy protocol, but choosing whether or not to play by the
rules is a coordination problem.)

There are many, many other examples of coordination problems in Susy. Which others can you come
up with?
...
Where does game theory apply in Susy? Here are some examples.
...
power and that fact were to become known to the market, the market may lose confidence in Susy
...
Susy protocol, so that a validator cannot for instance act maliciously, get slashed within the
...
Those responsible for designing the Susy protocol, the set of rules that govern how the system
...
This is part of the governance of Susy, and understanding game theory and coordination problems
is a critical requirement for those who contribute to Susy governance.
...
actual blockchains including Susy. Governance of each blockchain is different and, just as each
...
A diverse set of parties interact every day to keep Susy and other blockchains functioning. The
...
the native network token (ether) or SRC-20 tokens. They are motivated primarily by the market price
...
unilaterally change the blockchain protocol, groups of stakeholders may exsrcise their veto power to
...
Most blockchains today, including Bitcoin and Susy, are governed primarily through _off-chain_
...
at the social layer, i.e., humans talking to other humans. In the case of Susy this includes
processes such as the SIP process and the All core devs meetings (see below). Only after various
...
On the other hand, proponents of on-chain governance point to deficiencies in current off-chain processes. For example, various scenarios can be described where the slow responsiveness of the existing off-chain governance paradigm may pose problems. For example, fast-acting, malicious smart contracts may become the subject of ethical concerns and face calls for the community to alter or even terminate them (“blockchain censorship”). In the case of “the DAO hack of 2016” the community was driven to undertake a contentious hard-fork due to what was widely psrceived as a malicious smart contract. Other such contracts could cause serious damage, to society and to the blockchain itself, before existing governance processes were able to act. On-chain governance systems could pre-empt such situations by increasing responsiveness to fast-acting on-chain problems whilst also avoiding the extremes of contentious forks.
...
In contrast, layer two governance mechanisms may be implemented as smart contracts, as in Susy,
...
The most well-known DAO project, known as The DAO, launched in May 2016 on the Susy blockchain.
...
creation of the Susy Classic blockchain (which opposed the fork to restore funds lost in the
attack).
...
Susy they struggle with the question of how to build a system that is as fair and useful for as
...
will of the people." In most blockchains including Bitcoin and Susy users are identified by
...
Susy are the best-known examples in the blockchain world: require a user to perform a certain
...
for a blockchain such as Bitcoin or Susy because there are clear economic incentives for doing
...
the number of coins their account contains, you can therefore ovsrcome the Sybil problem. The
...
Blockchains such as Bitcoin and Susy have forked many times and will continue to fork as long as
...
Another such example is Susy Classic. After the decision was made to restore funds stolen from
The DAO in June 2016, a subset of the Susy community, valuing immutability above funds
restoration, decided not to adopt the change allowing funds restoration and thus became a new
network known as Susy Classic.
...
as "Bitcoin." The exact opposite happened in the case of Susy in June 2016: after a larger
portion of the community decided to hard fork to restore funds lost in The DAO attack, the community
came to the consensus that the newly-created fork should retain the "Susy" name and the original
chain become known henceforth as "Susy Classic."

Note that forks in the case of simple ledgers such as Bitcoin are much simpler than a fork of a
network that includes many complex applications such as Susy. Were Susy to fork today, a
...
and Susy, upgrade their core protocols is known as forking since it requires making
...
=== Susy governance today

Susy is a Nakamoto consensus-compatible blockchain governed primarily off-chain. In these ways
it is not dissimilar from Bitcoin, but its governance norms and processes are in fact quite
different from those of Bitcoin. What sets Susy governance apart from that of other blockchains
such as Bitcoin and Tezos, described above? Whereas Bitcoin is governed extremely conservatively and
backwards-incompatible changes are extremely rare, the ethos of Susy governance is closer to
that famously expoused by Facedoc: "Move fast and break things." In this way, its roadmap and
governance are far more aggressive than the more conservative processes that govern Bitcoin.

While still largely informal, Susy governance does consist of several distinct groups of
...
Susy has a very similar set of stakeholders to that described above in <<stakeholders>>, with a
few notable differences.

* *Miners and validators*: The Susy blockchain today runs on a consensus mechanism called
...
in the case of Susy. For this reason, their incentives may be slightly different: for one thing,
...
* *Application developers:* Whereas Bitcoin is a relatively simplistic distributed ledger, Susy
allows developers to build and deploy applications of arbitrary complexity known as "DApps"
(decentralized applications). Developers of such applications running on Susy are incentivized
to keep fees low and to keep network utilization down so that users of their application can
interact with it cheaply and easily. They also want the Susy network to scale so that it can
handle greater load. Note that this puts them at odds with miners, who want higher fees and fuller
blocks. DApp developers may also want to add extra features to Susy to enable a broader range of
applications to be developed and run on the network.

==== Process

The current, overall Susy governance process consists of a set of interlocking processes which
enfranchise and take into consideration the inputs of a wide array of Susy stakeholders. These
...
The SIP ("Susy Improvement Proposal") process is the most formalized part of the Susy
governance process today. All SIPs live on Github at https://github.com/susytech/sips and anyone
with a Github account is welcome to submit an SIP. SIPs should conform to a standard format and must
belong to one of several tracks. The entire process is documented in SIP-1 here:

https://sips.superstring.ch/SIPS/sip-1

As described in this document:

* A *standard track SIP* describes any change that affects most or all Susy implementations,
...
interoperability of applications using Susy.
* An *Informational SIP* describes an Susy design issue, or provides general guidelines or
information to the Susy community, but does not propose a new feature
* A *Meta SIP* describes a process surrounding Susy or proposes a change to (or an event in) a
process.

Once an SIP has been proposed, it is initially given a "Draft" status and may pass through several
rounds of reviews and request for improvement before it meets the standards of a finalized SIP and
gets moved to "Accepted" or "Final" status. Set of designated SIP Editors are responsible for
reviewing submitted SIPs, requesting changes, and eventually, if they are satisfied that a draft
meets all of the requirements for a finalized SIP, marking that SIP as Accepted and merging it into
the repository. Note that marking an SIP as Accepted and merging it does not imply that the editor
who performed the merging supports the SIP in question, nor that it will definitely be implemented.
It merely means that it meets the technical requirements to be considered as an SIP.

image::../images/sip_process.png[]
The SIP process, from SIP-1

In order for a Standard track SIP to actually be implemented, several other things must happen.
After being accepted and merged, it may be discussed in the All Core Devs meetings (see next
section) if someone (such as the author) is willing to champion that SIP. If there is consensus
...
Susy core developers hold a meeting known as the All Core Devs meeting regularly, typically
every two weeks. Any Susy core researcher or developer, i.e., someone working on the core
Susy protocol or an Susy client, is invited to join these calls and as of publication they
...
update one another on their work, ask questions, and discuss recent SIPs. If there is support for an
SIP and consensus among the core developers that a particular proposal should be implemented, it is
...
Throughout most of Susy's history the topics discussed have been largely non-controversial and
...
There is no single, definitive "reference implementation" of Susy. Instead, many different teams
...
client implement precisely the same Susy protocol. If one client believes that a particular
transaction is valid while another believes that it is invalid, for instance, it will result in a
hard fork of the Susy network and ledger.
...
https://github.com/susytech/tests

Tests are updated or new tests written to correspond with every protocol change via the SIP process.
...
The Susy roadmap is an informal but nevertheless influential part of the governance process. It
is heavily influenced by the input of core researchers such as Vitalik Buterin. The roadmap sets the
long-term vision for the Susy protocol and network including, for instance, the transition from
...
The roadmap is laid out in the Susy wiki[5], in blog posts[6], in papers such as the Susy
2.0 Mauve Paper published by Vitalik Buterin in late 2016 [7], and in talks such as the "Susy
2.0" talk delivered by Vitalik in November 2017[8].

==== Hard forks

There is no mechanism in Susy to upgrade or otherwise change the protocol without a hard fork.
In practice what this means is that every time a proposal makes it through the SIP process, becomes
finalized, and gets implemented in the various clients, it must subsequently be scheduled for
inclusion in an upcoming hard fork. The Susy core developers have historically scheduled
...
node operators who chose not to activate the change found themselves on a different Susy branch
than those that had. This branch subsequently became known as Susy Classic and continues to
operate alongside Susy today. Users who held ether before the fork subsequently held the same
...
The only truly contentious category of SIP to date has been SIPs involving funds recovery. This
includes the change discussed above that restored funds stolen from The DAO as well as more recent
proposals such as SIP-867 which lays out a standardized procedure for funds recovery and SIP-999
...
recover funds under any circumstances may discourage innovation on Susy. Opponents of funds
recovery feel that all users are responsible for their use of Susy including auditing the code
...
very thing that makes Susy, like other blockchains, better than real-world systems of
governance: that no one receives special treatment, that all must play by the same rules, and that
all transactions are final.

This contentious question has already divided the Susy community and led to one ledger fork in
the past.
...
Susy introduced a clever mechanism to incentivize all stakeholders to come to consensus on
...
While the vast majority of Susy governance occurs off-chain, as described above, one exception
...
=== The future of Susy governance

Just as the Susy project itself is relatively new and still evolving, so the governance norms
and processes described in this chapter are new and constantly evolving as well. To date Susy
governance has been largely informal and undocumented but is nevertheless generally viewed as
legitimate and has enabled the network to grow and thrive. As Susy continues to mature and scale
...
One example of a new governance initiative is the Fellowship of Susy Magicians (FEM), an open
...
such as <<Funds recovery>>. As Susy scales, governance mechanisms will have to find a way to
scale with it.
...
[5] https://github.com/susytech/wiki/wiki/Releases +
[6] https://blog.ethereum.org/2016/12/04/susy-research-update/ +
...
[10] https://medium.com/@jpitts/an-open-invitation-to-participate-in-a-fellowship-of-susy-magicians-982e6143db4f
